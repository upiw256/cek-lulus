...
export const getSKLDocument = async (data: any) => {
  ...
  await Promise.all([loadImage(imgKop), loadImage(imgTtd), loadImage(imgCap)]);
...
export const generateSKL = async (data: any) => {
  const doc = await getSKLDocument(data);
  doc.save(`SKL_${data.nisn}_${data.nama.split(' ')[0]}.pdf`);
};

export const generateSKLBase64 = async (data: any) => {
  const doc = await getSKLDocument(data);
